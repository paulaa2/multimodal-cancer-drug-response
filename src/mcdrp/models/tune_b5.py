"""Validation-based hyperparameter tuning for B5 hybrid GNN.

The grid is four candidates over learning rate and dropout. Architecture width
and depth stay at the B5 defaults so the search tests those defaults rather
than becoming a neural-architecture search.
...
Test is scored only for the selected candidate; the winner is not retrained.
"""
...
def tune_b5_for_split(
    cohort: pd.DataFrame,
    assignments: pd.DataFrame,
    graph_map: dict[str, Any],
    fingerprint_map: dict[str, Any],
    ...
    *,
    ...
    """Tune B5 on validation RMSE and evaluate the winner on test."""

    if run_split is None:
        from mcdrp.models.gnn_b5 import run_b5_for_split

        run_split = run_b5_for_split
    ...
    for params in iter_param_grid(DEFAULT_GRID):
        ...
        if not validation:
            raise RuntimeError(f"B5 {split_name} produced no validation rows.")
        rows.extend(validation)
        candidates.append((params, split_rows, split_frames))
        logger.info(
            "B5 %s %s validation RMSE %.3f",
            split_name,
            params,
            validation[0]["rmse"],
        )
...
def tune_b5(
    ...
    output: str | Path = "results/models/b5_tuning_metrics.csv",
    summary: str | Path = "data/reports/b5_tuning_summary.json",
    predictions_output: str | Path = "results/predictions/b5_tuned.csv",
    *,
    target: str = "ln_ic50",
    ...
    """Tune B5 hyperparameters for all requested splits."""

    torch_device = resolve_torch_device(device)
    if torch_device is None:
        raise RuntimeError("B5 tuning requires PyTorch.")

    from mcdrp.models.gnn_b5 import build_fingerprint_map
    ...
    for split_name in split_names:
        logger.info("=== Tuning B5 split: %s ===", split_name)
        ...
        split_rows, split_frames = tune_b5_for_split(
            cohort,
            assignments,
            graph_map,
            fingerprint_map,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Tune B5 hybrid GNN hyperparameters.")
    ...
    parser.add_argument("--output", default="results/models/b5_tuning_metrics.csv")
    parser.add_argument("--summary", default="data/reports/b5_tuning_summary.json")
    parser.add_argument("--predictions-output", default="results/predictions/b5_tuned.csv")
...
def main() -> None:
    ...
    print(f"Wrote B5 tuning metrics to {args.output}")
