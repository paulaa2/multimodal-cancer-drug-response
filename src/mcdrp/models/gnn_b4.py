"""B4 initial multimodal GNN baseline.
...
It is intentionally modest and dependency-light. It does not use PyTorch
Geometric yet; that can come after this baseline proves the graph path is wired
correctly.
"""
    ...
    def __init__(
        ...
        *,
        ...
        for row in self.rows.itertuples(index=False):
            ...
            self.targets.append(np.float32(getattr(row, target)))
...
class GraphConvBlock(nn.Module):
    """Simple mean-aggregation graph convolution block."""
    ...
    def __init__(
        ...
        *,
        ...
        self.regressor = nn.Sequential(
            nn.Linear(graph_hidden_dim + cell_hidden_dim, fusion_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
...
def evaluate(
    ...
    *,
    ...
    """Predict a loader and return y_true, y_pred, and mean loss."""
    ...
    return np.concatenate(targets), np.concatenate(predictions), float(np.mean(losses))
...
def run_b4_for_split(
    ...
    *,
    ...
    for subset, loader in (("validation", val_loader), ("test", test_loader)):
        y_true, y_pred, _loss = evaluate(model, loader, device=device)
        row = metric_row(
            "gnn",
            split_name,
            subset,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Run B4 initial GNN baseline.")
